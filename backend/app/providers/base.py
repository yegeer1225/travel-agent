...
class AmapProvider(Protocol):
    ...
    async def search_poi(
        ...
        **返回空列表 ≠ 出错**：高德对搜不到的关键词就是返回 `pois: []`，
        ...
        `center`（`"lng,lat"`）：**排序中心点**，给泛关键词（"公园""火锅"）用。

        不加也返回得出来，但落点由高德自己决定 —— 实测成都搜「公园」返回的
        全是高新区那批（中位距市中心 **10.7 km**）；给个市中心附近的坐标后
        变成青羊区的（**2.2 km**）。所以它是**排序权重不是硬过滤**：
        实测搜「都江堰」（56 km 外）时给 `center="天府广场"` 返回与不加**完全一致**
        （重合 20/20，目标仍排第 1）→ 无"搜不到远处目的地"的副作用。

        ⚠️ **别拿 geocode 的坐标当 center**：`/v3/geocode/geo` 查"成都"给的是
        **市政府驻地**（104.066301,30.572961），而成都市政府 2010 年搬到了高新区 ——
        用它当中心点返回的**还是高新区那批**，等于没加。实测见 `DECISIONS.md` D75。
        """
        ...
