...
· 校验 `status` 的合法取值是 **三态 `pass / fail / unknown`**（内部需求文档 6.2、内部设计规范 2.1）。
...
class AuthorType(StrEnum):
    ...
    因为 DB 层 `guides.user_id` 是 NOT NULL、也没有这一列（见内部技术方案 3.3 的
...
class Day(BaseModel):
    ...
    ⚠️ 这是 **M0 冻结后的一次契约变更**，理由和时点都记在 `D37`：
    变更发生在**前端动手之前**，成本最低；不加这个字段，
    内部需求文档 6.1 里那两条硬判据**没有地方安放**。"""
...
class Trip(BaseModel):
    ...
    必须用 `owned_trip()` 直接按 `trips.user_id` 校验（见 内部技术方案 第三节末）。"""
    ...
    变更时点同 D37：**发生在前端动手之前**，成本最低。理由见 D46。"""
...
class LikeState(BaseModel):
    ...
    target_id: str
    liked: bool
    count: int = Field(ge=0)  # 先 `COUNT(*)` 现算，不加冗余字段（见 内部技术方案 三节）
