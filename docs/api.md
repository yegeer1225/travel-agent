...
⚠️ **`/spots` 这一对（search + detail）都落在"要登录"那侧**：
游客点导航栏「旅游景点」→ 页面发 `/spots/search` → **立刻 401，页面是空的**。
这跟"导航栏各入口都允许进入"是冲突的，处理方式见 1.1.4。
...
### 1.1.3 401 的统一处理：弹窗 + 登录后继续原操作

🔴 **不要跳页。** 游客点景点详情 → 401 → 跳 `/login` → 登录完回到首页 ——
他刚想看的东西丢了，得重新找一遍。**这是把"注册引导"做成了"惩罚"。**
...
5. 🔴 弹窗的开/关**由用户动作驱动**，不能在"发请求时"设置 ——
   否则 React 19 严格模式 `useEffect` 跑两次会开两个弹窗（同 6.3 的 SSE 清理问题）

⚠️ 目前的 `handleUnauthorized()` 是「清 token → 跳登录」，**要按上面改**。

### 1.1.4 ⏳ 待拍板：`/spots` 那一对要不要拆开

现状（见 1.1.2）：**搜索和详情都要登录** → 游客点导航栏「旅游景点」拿到空页面。

| 选项 | 改什么 | 代价 |
|---|---|---|
| **A. 拆开** | `GET /spots/search` 改公开，`GET /spots/{id}` 保持要登录 | 后端去掉 1 个 `Depends`；但**限流键现在用 `user_id`** —— 匿名搜索没有它，得改成 IP，那要和 `login` 的 IP 限流合起来看 |
| **B. 不拆** | 什么都不改，游客点「旅游景点」就弹登录 | 与"导航栏各入口都允许进入"矛盾；但对单机作品无害 |

**未定之前前端不做特殊处理**（`/spots` 页照旧，401 走 1.1.3 的统一弹窗）。
...
| 401 | `unauthorized` | token 缺失/过期/签名错（M9 起） | 清本地 token → **弹全局登录弹窗（不跳页）**，登录后继续原操作 —— 见 1.1.3 |
