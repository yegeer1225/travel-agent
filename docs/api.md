...
⚠️ 目前的 `handleUnauthorized()` 会**清 token + `window.location.assign('/login')`**（整页刷新），
**要按上面改成开弹窗** —— 它是唯一需要动的那个函数。
...
- `Spots.tsx:87-95`：关键词为空时**直接 return、不发请求** → 游客点「旅游景点」**不会撞 401**，
  看到的是搜索表单 + 引导文案，**不是空白页**
- 只有**提交搜索**或**点景点详情**才需要登录 —— 这正好是需求要的"浏览自由、用功能弹登录"

→ 前端**不做任何特殊处理**：401 全走 1.1.3 的统一弹窗。
→ 后端**零改动**。
