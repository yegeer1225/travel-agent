...
backend/tests/    614 个用例（`pytest tests/ -q`，2026-09-20 实测全绿）
...
放在这里是因为它们**有代价、有取舍**，不是"用了什么库"这种描述。
...
下面三条是完整走过"定方案 → 实测 → 认代价"的，不是拍脑袋决定的。

**① 性能：把 85% 的耗时从 LLM 决策轮里砍掉**

一次规划原本要 9 分 14 秒。拆账后发现 **85% 的耗时在 LLM 调用**，其中"子 agent 出关键词 → 搜 → 看结果"的决策轮占了 **12~20 次调用**；而真正的高德搜索只花 **~15 秒**。

慢的不是工具，是「决定搜什么」的 LLM 往返。

改法：删掉搜索子 agent，让主 agent **第一轮就一次性派出全部搜索**（5~12 个关键词覆盖景点/美食/住宿），同批并行执行。结果 **9m14s → 3m55s（-58%）**，LLM 调用 15+ → 6 次。

代价：模型失去"看结果换词"的临场探索能力 → 用「允许补搜一轮 + 关键词台账防重复」兜底（换词本就是低风险决策，规则够用）。

**② 反例：一次做了、量了、否了的优化**

想着把行程生成拆成"分配 + 各天并行填充"，理论上该更快。实测（同需求各跑一次端到端）：
...
用未改动的另一阶段做对照校准服务端方差后，**结论是持平，零收益** → 回退。

根因比数字值钱：慢档模型**每次调用的固定开销约 20~35 秒**（网络 + 排队 + 首 token），拆成 6 次调用 = 固定开销 ×6。**在慢档模型上，唯一的耗时杠杆是「调用次数」，"减小单次输出"是伪杠杆**——这条结论同时预言了换快档后两段式才可能有效，但那时总量已进 1 分钟，不值得复杂度。

**③ 数据源：为什么"搜不到"时宁可是空的，也不回落**

景点页搜的是**本站收录库**（真实高德 POI 的快照），库里没有就返回空列表，**不静默换成实时搜索结果**。

因为回落会让同一页混两种来源（前端没法解释"这条为什么来自高德"），更糟的是它把「**确实没收录**」这条真实路径**掩盖掉**。记录只能由脚本从真实接口写入——**绝不手写 INSERT**（手写就是编数据，被问"这数据哪来的"答不上）。

代价也认：收录库是静态快照会过期（每行记采集时间，重跑即刷新）；clone 下来没跑收录脚本的话，这一页永远是空的——所以 README 把它列为**必做步骤**。
...
这是一个"文档先于代码"的项目：契约与决策先落文字，再写实现。仓库内的文档各管一段：

| 文件 | 只回答 |
|---|---|
| `README.md` | 这个项目是什么、怎么跑起来（本页） |
| `AGENTS.md` | **AI 协作入口**：项目一句话 / 目录地图 / 铁律速查 —— 任何 AI 进仓库干活先读这页 |
| `docs/api.md` | 接口契约（38 个端点 / 10 种 SSE 事件 / 错误码表，已冻结） |
| `docs/types.ts` | 前端类型 —— 由 `schemas.py` 自动生成，不手抄 |
| `docs/sample_trip.json` | 一次真实生成的行程输出样例 |
| `eval/README.md` | 评测怎么跑、case 怎么写 |
| `docs/screenshots/` | 界面截图（见开头「先看长什么样」） |

> 开发过程中的决策记录、需求方案、模块设计与交接清单不在本仓库（属于内部工作文档，不随代码公开）。
> 本 README 的「九、当前边界」与「七、几个工程点」已把其中**对外有意义的部分**浓缩出来了。
