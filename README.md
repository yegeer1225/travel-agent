...
| **数据源档位** | 两档，前端无感知（D23）：**`real`** = 真高德，任何城市都行（2026-09-17 实测成都 / 杭州双城正常，含距离与天气）；**`mock`** = 内置 9 个 POI，**只覆盖成都** → 非成都目的地排不出行程（子代理反复空搜、耗到轮数上限约 159s 才报错，且文案是技术黑话 —— 根因是 `mock.py:397` 的城市过滤，**不是模型空转**）。`.env.example` 模板默认 `mock`（**别人 clone 下来零 Key 可跑**）；要真数据改 `real` + 填 `AMAP_WEBSERVICE_KEY` |
| **单次生成耗时** | ⚠️ 实测区间 **146~371s**（3 天行程，real 档，端到端 SSE）：成都 146s／杭州 **371s**。早前固定输入连跑 3 次均成功（198~274s）。**稳定性没问题，方差是主要风险**；方差根因是**模型派几个 `task` 子代理**（成都 2 个／杭州 7 个，其中 3 个近乎重复）。瓶颈是 LLM 调用次数，不是数据接口。DeepSeek 付费档快一个量级 |
